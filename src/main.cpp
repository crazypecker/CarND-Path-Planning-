...
int NextWaypoint(double x, double y, double theta, vector<double> maps_x, vector<double> maps_y, vector<double> maps_dx, vector<double> maps_dy)
{
 ...
 // vector into the direction of the road (perpendicular to normal vector)
  double vx = -nx;
  double vy = -ny;
  
  //dot product 
  //(if +ve behind the waypoint[no increment])
  //if -ve beyond the waypoint[increment the waypoint]
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            //create list of wide spread (x,y) waypoints evenly spaced at 30 m
            //later we will interpolate these waypoints using spline and fill it in with more points to control speed
            ...
            //calculate how to break up spline points so that we travel at our desired ref vel
            ...
            json msgJson;
          	...
          	//static vector<double> next_x_vals;
          	//static vector<double> next_y_vals;
            ...
            double dist_inc = 0.3; //0.5m distance gives velocity close to 50 MPH
            for(int i = 0; i < 50; i++)
            {
